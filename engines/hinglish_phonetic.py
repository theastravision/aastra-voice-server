"""Dynamic roman phonetic hyphenation for Hinglish/Hindi TTS and STT denormalization."""
...
_PHONETIC_MUTATIONS: tuple[tuple[re.Pattern[str], str], ...] = (
    (re.compile(r'ci', re.IGNORECASE), 'sa-i'),
...
def _load_json_maps_cached() -> tuple[dict[str, str], dict[str, str]]:
    ...
    try:
        ...
        logger.warning('Failed to load phonetic hyphen map: %s', exc)
        return {}, {}
    loanwords = {str(k).lower(): str(v) for k, v in (data.get('loanwords') or {}).items()}
    hindi = {str(k).lower(): str(v) for k, v in (data.get('hindi_roman') or {}).items()}
...
def _load_skip_names() -> frozenset[str]:
    global _NAMES_MTIME
    path = _names_path()
    if path.is_file():
        mtime = path.stat().st_mtime
        if mtime != _NAMES_MTIME:
            _load_skip_names_cached.cache_clear()
            _NAMES_MTIME = mtime
    return _load_skip_names_cached()
...
def _transform_word_cached(lowered: str) -> str:
    ...
    return result or lowered


class HinglishPhoneticEngine:
    """Deterministic roman syllable hyphenation for Hinglish/Hindi TTS."""
    ...
    def process_sentence(self, sentence: str) -> str:
        ...
        return _WORD.sub(_replace, sentence)
...
def apply_phonetic_hyphens(text: str, *, reply_script: str = 'hinglish') -> str:
    """
    Hyphenate roman tokens for clearer F5/Swara articulation.
...
def _reverse_glossary() -> dict[str, str]:
    _load_json_maps()
    return _reverse_glossary_cached()
...
def denormalize_phonetic_word(word: str) -> str:
    """Map a hyphenated phonetic token back to canonical roman spelling."""
...
def denormalize_phonetic_text(text: str, *, reply_script: str | None = None) -> str:
    """
    Reverse TTS phonetic hyphenation in STT transcripts for hi/hinglish sessions.
    Restores glossary loanwords and joins dynamic syllable hyphens.
    """
    ...
    return _WORD.sub(_replace, cleaned)
