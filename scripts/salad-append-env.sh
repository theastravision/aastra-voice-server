...
# On repeat runs, delegates to sync-env.sh to refresh pipeline tuning keys.
set -euo pipefail
...
if grep -qF "$MARKER" "$ENV_FILE" 2>/dev/null; then
  echo ".env already has Salad block — refreshing pipeline keys via sync-env.sh"
...
STREAM_STT_MIN_CHARS=4
STT_UTTERANCE_MAX_SECS=90
STT_TRANSCRIBE_TIMEOUT_SECS=120
...
echo "  F5_REF_AUDIO_EN / F5_REF_AUDIO_HINGLISH — see data/voices.json"
