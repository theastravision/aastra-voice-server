"""Tests for dynamic Hinglish phonetic hyphenation and STT denormalization."""
...
def test_glossary_technology():
    assert apply_phonetic_hyphens('technology', reply_script='hinglish') == 'tek-no-lo-jee'
...
def test_dynamic_unseen_word_gets_hyphens():
...
def test_already_hyphenated_unchanged():
...
def test_mixed_sentence_dynamic_and_glossary():
    ...
    assert 'con-' in result or 'config' not in result


def test_stt_denorm_glossary_word():
    assert denormalize_phonetic_word('tek-no-lo-jee') == 'technology'
    assert denormalize_phonetic_word('dee-sa-eed') == 'decide'
...
def test_stt_denorm_sentence_roundtrip():
    ...
    assert 'configura' in restored.lower()
    assert 'management' in restored.lower()
